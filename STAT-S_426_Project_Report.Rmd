...
Due to the Subprime mortgage crisis that happened in US in the year of 2007, the fluctations of return are much larger than the return in 2006, which strengthens the difficulty on the precision of model prediction and inference. 
...
The density plots for the total R and R for 2006 and 2007 are pretty much similiar, the number of R in 2006 and total plots of each year are left skewed and the large part of data is greater than the mean zero. The value of R in 2007 looks quite normal and they both look similar than multinormal distribution. Combining with the traceplot of R, 
...
For a two-mixture normal model, we set up some information for $\mu=(0.008,-0.012)$ , 
$\sigma^{2}=(1e-04,2e-04)$  and $\pi$ = (`r p`). We will begin our following computations based on this prior information.
...
plot(trace.matrix[,1],trace.matrix[,2],ylab=expression(mu[2]),xlab=expression(mu[1]),
     main = expression('Joint posterior of'~mu))
```

We can see from the plot that $\mu_1$ and $\mu_2$ have negative linear correlation, it proves the results 
that the sum of weighted expected value is equal to zero so that the $\mu$s' need to have opposite sides. 
...
The plot of posterior density of $\mu$ shows the peak of $\mu_2$ is slightly less than zero and the peak of $\mu_1$ 
is greater than zero. And the density of $\mu_2$ is more condensed. This may be due to the proportion of the second 
normal model with negative $\mu$ i.e. $\pi_2$ is greater.

The plot of posterior density of $\pi$ is symmetric with respect to 0.5, since the sum of $\pi_1$ and $\pi_2$ equals
 to 1. The peak of $\pi_2$ is slightly greater than 0.6 which implies the weight of the second normal model is 
larger. This shows in the era of financial crisis, it's more likely to lose money due to the larger possibility 
of negative return rate.
...
```{r acf effective sample size and plot,fig.cap="Autocorrelation for mu1 and mu2"}
...
(1) In the years of 2006 and 2007, the negative component of mixture normal donomiates the entire model because of 
its larger $\pi_2$. This implies the larger chance of losing money in options investment. The posterior proportion 
is contradictory to our prior information that we assume the weight of positive normal ought to be greater. However, 
...
(2) Some disadvantages of our mixture model: Due to the complexity of financial market, the two component normal 
mixture model may not be precise enough to reflect the behaviors of industry. An alternative way is to break down 
positive and negative normal models into several sub normal models and devote more weights into these new models. 
...
(3) The relation among variances on each day: We utilize the equation between $\sigma^2_{k,t}$ and $\sigma^2_{k,t-1}$
 from Rombouts and Stentoft's paper where there are four coefficents controlling the growth of variance. 
 The effectiveness of this equation remains in doubt, we can not figure out the reasons of setting up that equation. 

(4) Limited stress on time series data: We conduct many techniques in Bayesian Statistics especially in Markov 
Chain Monte Carlo Method. Unfortunately, we may overlook several assumptions or basic methods of analyzing 
time series data. This could be a further direction we can work on to improve the accuracy of our model.
