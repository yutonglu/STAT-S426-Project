...
```{r plot of R,fig.cap="The return process of R",echo=FALSE,fig.pos='H'}
...
For a two-mixture normal model, we randomly set up some information for $\mu=(0.008,-0.012)$ , $\sigma^{2}=(1e-04,2e-04)$  
...
We can see from the plot that $\mu_1$ and $\mu_2$ have negative linear correlation,it proofs the results which shown that the sum of weighted expected value is equal to 0 so that the $\mu$ need to have opposite value. 
...
lines(density(trace.matrix[,2]), type = 'l',col="seagreen2")
legend("topleft",legend=c(expression(mu[1]),expression(mu[2])),col = c("steelblue1","seagreen2"), lty=1,cex=1,bty="n")


plot(density(trace.matrix[,11]), type = 'l', ylab = 'Density', xlab = expression(pi),
     xlim = c(0.2,0.8), las = 1,
     main = expression('Posterior density of'~pi),col="orange")
lines(density(1-trace.matrix[,11]), col="orchid1")
legend("topleft",legend=c(expression(mu[1]),expression(mu[2])),col = c("orange","orchid1"), lty=1,cex=1,bty="n")
