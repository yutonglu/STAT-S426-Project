...
Thus, the value of $\sigma_{k,t}^2$ depends on parameters $\theta_k=(\omega_k,\gamma_k,\alpha_k,\beta_k)$ with $\omega_k>0,\alpha_k≥0,\beta_k≥0$.
...
n.iter = 150 # number of iterations for Gibbs
...
log.likeli.ratio.1 = function(para1, para2, data){
 ...
 likeli.1 = log(dnorm(data, mu[1] + rho_t, var.list.1))
 likeli.2 = log(dnorm(data, mu[1] + rho_t, var.list.2))
...
log.likeli.ratio.2 = function(para1, para2, data){
 var.list.1 = generate.var(para1,var(data),data)
 var.list.2 = generate.var(para2,var(data),data)
 likeli.1 = log(dnorm(data, mu[2] + rho_t, var.list.1))
 likeli.2 = log(dnorm(data, mu[2] + rho_t, var.list.2))
 return(sum(likeli.1-likeli.2))
}
...
for (i in 1:n.iter){
  ...
  b = new.p/(1-new.p)*sum(rnorm(1, mean = 0, sqrt(var.2))/var.2) - sum(rnorm(1, mean = 0, sqrt(var.1))/var.1)
  
  new.mu1 = abs(rnorm(1,-A^(-1)*b,A^(-1)))
...
plot(density(trace.matrix[,1]), type = 'l', ylab = 'Density', xlab = expression(mu),
     las = 1,xlim = c(-0.15, 0.15), ylim = c(0, 22),
